...
static bool talker(std::string &model_name, std::string &user_query) {
  ...
  body_json["messages"] =
      json::array({{{"role", "user"}, {"content", user_query}}});

  std::string body = body_json.dump();

  auto result = httplib::stream::Post(cli, "/v1/chat/completions", body,
                                      "application/json");

  if (result) {
    std::ofstream out("model_output.json");
    while (result.next()) {
      // std::cout.write(result.data(), result.size()); //throws llm response to
      // terminal
      out << result.data();
    }
    out.close();
