...
from llm_interface.qwen_runner import LLMError, stream_code_review, stream_follow_up
        ...
        try:
            ...
            def content_callback(content: str):
                if not self._is_cancelled:
                    ...
                    QApplication.postEvent(self.signals, ContentReadyEvent(content))
            ...
            logger.warning(
                f"Failed to initialize C++ streamer: {e}. Using Python fallback."
            )
            self._setup_python_fallback()
...
class FollowUpTask(BaseReviewTask):
    ...
    def run(self):
        ...
        try:
            ...
            for token in stream_follow_up(self.original_review, self.question):
                ...
                if token_count % 30 == 0:
                    self.signals.progress.emit(
                        f"Processing response... ({token_count} tokens)"
                    )
            ...
            self.signals.finished.emit()
            logger.info(
                f"FollowUpTask completed successfully with {token_count} tokens"
            )
...
class TaskManager:
    ...
    def start_followup(
        self, original_review: str, question: str, signals_target
    ) -> FollowUpTask:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "reviews": sum(
                1 for task_type, _ in self.task_history if task_type == "review"
            ),
            "followups": sum(
                1 for task_type, _ in self.task_history if task_type == "followup"
            ),
