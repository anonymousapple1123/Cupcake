...
int main() {
  ...
  if (run_cmd("which clang++ >/dev/null 2>&1") != 0) {
    ...
    // This opens the softwareupdate installation flow; it may require user
    // interaction.
    if (run_cmd("xcode-select --install >/dev/null 2>&1 || true") != 0) {
      std::cerr << "Failed to invoke xcode-select --install (already installed "
                   "or manual intervention required).\n";
    } else {
      std::cout << "Requested Command Line Tools install. Wait for completion "
                   "then re-run this program if needed.\n";
    }
  } else {
  ...
  if (run_cmd("which brew >/dev/null 2>&1") != 0) {
    std::cout << "Homebrew not found. Installing Homebrew (may request "
                 "password)...\n";
    // Official Homebrew install script
    if (run_cmd("/bin/bash -c \"$(curl -fsSL "
  ...
  std::cout
      << "5) Installing Ollama via official installer script (curl | sh)...\n";
  // Use the recommended curl installer provided by Ollama
  if (run_cmd("/bin/sh -c \"curl -fsSL https://ollama.com/install.sh | sh\"") !=
      0) {
  ...
  std::cout << "6) Ensure ollama CLI is available in PATH (may require new "
               "shell)...\n";
  if (run_cmd("which ollama >/dev/null 2>&1") != 0) {
    std::cout << "ollama not found in PATH. Attempting to source common "
                 "install locations and retry...\n";
    // Try common locations added by installer
    run_cmd("export "
            "PATH=\"$HOME/.local/bin:$HOME/.ollama/bin:/usr/local/bin:/opt/"
            "homebrew/bin:$PATH\" && which ollama >/dev/null 2>&1");
    if (run_cmd("which ollama >/dev/null 2>&1") != 0) {
      std::cerr << "ollama still not found. You may need to open a new "
                   "terminal or add ollama to PATH, then re-run.\n";
    }
  } else {
  ...
  if (run_cmd("ollama pull qwen2.5-coder") != 0) {
    std::cerr << "Failed to pull qwen2.5-coder. Check network, model name, or "
