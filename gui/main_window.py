...
class MainWindow(QMainWindow):
    ...
    def _select_file(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "File Selection Error", f"Failed to select file: {e}"
            )
    ...
    def _load_selected_file(self, file_path: str):
        ...
        try:
            ...
            if not file_info.get("is_valid_python", False):
                QMessageBox.warning(
                    self, "Invalid File", "Please select a valid Python file."
                )
                return
            ...
            QMessageBox.critical(
                self, "File Load Error", f"Failed to load file information: {e}"
            )
    ...
    def _ask_followup(self):
        ...
        if not question:
            QMessageBox.warning(
                self, "Empty Question", "Please enter a follow-up question."
            )
            return
        ...
        try:
            ...
            task = self.task_manager.start_followup(
                self.full_response_text, question, self
            )
            self.threadpool.start(task)
            ...
            QMessageBox.critical(
                self, "Follow-up Error", f"Failed to process question: {e}"
            )
        ...
        try:
            if llm_client.test_connection():
                ...
                QMessageBox.information(
                    self, "Connection Test", "Successfully connected to LLM service!"
                )
            else:
                self.connection_status.setText("LLM: ✗ Failed")
                self.connection_status.setStyleSheet("color: red;")
                QMessageBox.warning(
                    self, "Connection Test", "Failed to connect to LLM service."
                )
        except Exception as e:
    ...
    def _set_processing_state(self, is_processing: bool, status_text: str = ""):
        ...
        self.followup_input.setEnabled(
            not is_processing and bool(self.full_response_text)
        )
